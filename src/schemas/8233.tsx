...
export const US_TINSchema = () => {
  return Yup.object().shape({
    usTINTypeId: Yup.number().min(1).required("Please select"),
    ...
    reasionForForegionTIN_NotAvailable: Yup.string().required("please Enter reason for non-availability"),
  });
};
...
export const ownerSchema = () => {
  return Yup.object().shape({
    exemptionApplicableForCompensationForCalnderYear: Yup.number().min(1).required(),
...
export const amountSchema = () => {
  return Yup.object().shape({
    ...
    taxTreaty_TotalCompensationYouExpectForThisCalenderYear:Yup.number().min(1).required(),
    taxTreaty_TreatyId:Yup.number().min(1).required(),
    taxTreaty_TreatyArticleId:Yup.number().min(1).required(),
    taxTreaty_TotalCompensationListedon11bExemptFromTax:Yup.number().min(1).required(),
    taxTreaty_CheckAll:Yup.boolean().oneOf(
        [true],
        "Please mark the checkbox"),
        taxTreaty_CountryOfResidenceId:Yup.number().min(1).required(),
        taxTreaty_NoncompensatoryScholarshiporFellowshipIncome:Yup.number().min(1).required(),
        taxTreatyAndTreatyArticleOnWhich_BasingExemptionFromWithholdingTreatyID:Yup.number().min(1).required(),
        taxTreatyAndTreatyArticleOnWhich_BasingExemptionFromWithholdingArticleID:Yup.number().min(1).required(),
        totalIncomeListedIncomeonLine13ATaxExemptAmount:Yup.number().min(1).required(),
...
export const certificateSchema = () => {
  return Yup.object().shape({
    i_Certify_BeneficialOwnerOrAuthorisedToSignForAllMentionIncome: Yup.boolean().oneOf(
      [true],
      "Please mark the checkbox"
    ),
    ...
    i_Certify_BeneficialOwnerResidentOfTreatyCountryOf12A_13B: Yup.boolean().oneOf(
      [true],
      "Please mark the checkbox"
    ),
    i_Certify_FurthermoreIAuthorise: Yup.boolean().oneOf([true], "Please mark the checkbox"),
...
export const partCertiSchema = () => {
  return Yup.object().shape({
    signBy: Yup.string().required("Please enter "),
    enterConfirmationCode: Yup.string().required("Please enter code"),
    signDate: Yup.date().required("Please enter date"),
...
export const declarationsSchema = () => {
  return Yup.object().shape({
    isDeclaration: Yup.boolean().oneOf(
      [true],
      "Please mark the checkbox"
    ),
    isConsentRecipent: Yup.boolean().oneOf(
      [true],
      "Please mark the checkbox"
    ),
